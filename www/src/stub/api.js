...
const mock = function (app, server, compiler) {
  ...
  app.post('/api/domain_delete', function (req, res) {
    state.user.domains = state.user.domains.filter(v => {
      return v.user_domain !== req.body.user_domain
    })
    res.json({})
  })
  app.post('/api/set_subscribed', function (req, res) {
    state.user.unsubscribed = !req.body.subscribed
    res.json({})
  })
  app.post('/api/user_delete', function (req, res) {
    res.json({})
  })
